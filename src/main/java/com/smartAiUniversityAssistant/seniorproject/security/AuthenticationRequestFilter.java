...
public class AuthenticationRequestFilter extends OncePerRequestFilter {
    ...
    @Override protected void doFilterInternal(HttpServletRequest request,HttpServletResponse response,FilterChain chain) throws ServletException,IOException {
        ...
        boolean usersMutation=usersPath && (request.getMethod().equals("POST") || request.getMethod().equals("PUT") || request.getMethod().equals("PATCH"));
        long start=System.nanoTime();
        try {
            if (authPath || usersPath) response.setHeader("Cache-Control","no-store");
            if (request.getMethod().equals("POST") && (path.equals("/api/v1/admin/auth/login") || path.equals("/api/v1/admin/auth/refresh") || path.equals("/api/v1/admin/auth/change-password")) || usersMutation) {
                ...
                if (usersPath && bytes.length>16384) throw new AuthenticationFailure(413,"CONTENT_TOO_LARGE","The request body is too large.");
